...
computeRelativePath() 
{
  ...
  local MakeRelativeArray=($MakeRelative)
  local StartingDirArray=($StartingDir)

  IFS=$OLDIFS

  # Get the length of the two arrays
  local MakeRelativeArrayLength=${#MakeRelativeArray[@]}
  local StartingDirArrayLength=${#StartingDirArray[@]}

  # First skip the parts of the paths that are the same
  local i
  for (( i = 0; i < $MakeRelativeArrayLength && i < $StartingDirArrayLength; i++ ))
  do
    if [ "${MakeRelativeArray[$i]}" != "${StartingDirArray[$i]}" ]
    then
      break
    fi
  done
  ...
  do
    AppendToEnd="${AppendToEnd}${MakeRelativeArray[$j]}/"
  done
...
GetDefaultJDK()
{
  ...
  #  4) A java in /usr/java/jdk7*
  #  5) A java in /usr/java/jdk6*
  #  6) A java in /usr/java7* (AIX)
  #  7) A java in /usr/java6* (AIX)
  #  8) Any java on the PATH.
    ...
    then
      ...
      then
        ...
        then
          ...
          # See if we can find /usr/java/jdk7*
          jdk=`ls /usr/java 2>/dev/null | grep jdk7 | sort -r | head -1`
          ...
          then
            ...
            then
              ...
              # See if we can find /usr/java7*
              jdk=`ls /usr 2>/dev/null | grep java7 | sort -r | head -1`
